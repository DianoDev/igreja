...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    const props = __props;
    const acao = ref("/admin/estatuto/create");
    inject("events");
    ...
    const loadData = async () => {
      try {
        const response = await axios.get(`/admin/estatuto/${props.data.id}`);
        acao.value = "/admin/estatuto/";
        ...
        toast.error("Não foi possível recuperar os dados do Estatuto.");
      }
    };
    onMounted(() => {
      var _a;
      if ((_a = props.data) == null ? void 0 : _a.id) {
        acao.value = `/admin/estatuto/${props.data.id}`;
        loadData();
      }
      ready.value = true;
    });
      ...
      if (ready.value) {
        ...
        if (!readOnly.value) {
          ...
          if (!arquivoExistente.value || arquivoRemovido.value) {
            ...
            _push(`<div class="mt-1"><label for="arquivo" class="flex items-center justify-center w-full px-4 py-3 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors"><div class="text-center"><i class="fa fa-upload text-gray-400 text-2xl mb-2"></i><p class="text-sm text-gray-600"><span class="font-semibold text-blue-600">Clique para selecionar</span> ou arraste o arquivo aqui </p><p class="text-xs text-gray-500 mt-1"> PDF, DOC, DOCX (máx. 10MB) </p></div></label><input type="file" id="arquivo" accept=".pdf,.doc,.docx" class="hidden"></div>`);
            if (selectedFile.value) {
              _push(`<div class="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-md"><div class="flex items-center justify-between"><div class="flex items-center"><i class="fa fa-file text-blue-600 mr-2"></i><span class="text-sm text-blue-800">${ssrInterpolate(selectedFileName.value)}</span></div><button type="button" class="text-red-600 hover:text-red-800 text-sm" title="Remover arquivo"><i class="fa fa-times"></i></button></div></div>`);
            } else {
              _push(`<!---->`);
            }
            _push(ssrRenderComponent(_sfc_main$3, {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/Estatuto/EstatutoForm.vue");
