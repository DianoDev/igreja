...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    const props = __props;
    const acao = ref("/admin/pessoas/create");
    inject("events");
    ...
    const loadData = async () => {
      try {
        ...
        toast.error("Não foi possível recuperar os dados do  Pessoa.");
      } finally {
        ready.value = true;
      }
    };
    onMounted(async () => {
      ...
      if ((_a = props.data) == null ? void 0 : _a.id) {
        acao.value = `/admin/pessoas/${props.data.id}`;
        await loadData();
      } else {
        ready.value = true;
      }
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (ready.value) {
        ...
        _push(ssrRenderComponent(_sfc_main$2, {
          id: "cpf",
          class: "w-full",
          modelValue: form.value.cpf,
          "onUpdate:modelValue": ($event) => form.value.cpf = $event,
          disabled: readOnly.value
        }, null, _parent));
        _push(ssrRenderComponent(_sfc_main$3, {
          message: errors.value.cpf
        }, null, _parent));
        _push(`</div><div class="mb-4">`);
        _push(ssrRenderComponent(_sfc_main$1, {
          for: "telefone",
        ...
        _push(ssrRenderComponent(_sfc_main$2, {
          id: "telefone",
          class: "w-full",
          modelValue: form.value.telefone,
          "onUpdate:modelValue": ($event) => form.value.telefone = $event,
          disabled: readOnly.value
        }, null, _parent));
        _push(ssrRenderComponent(_sfc_main$3, {
          message: errors.value.telefone
        }, null, _parent));
        ...
        if (!readOnly.value) {
          _push(`<div class="flex justify-center space-x-2"><button type="submit" class="px-4 py-2 text-white rounded-md bg-blue-500 hover:bg-primary-hover"${ssrIncludeBooleanAttr(processing.value) ? " disabled" : ""}>`);
          if (!processing.value) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Admin/Pessoa/PessoaForm.vue");
