    ...
    axios.post(acao.value, form.value)
        ...
        .catch(error => {
            ...
            if (error.response) {
                ...
                const message = data.message || "Ocorreu um erro ao salvar  Cardapio.";
                handleError(message);
            } else {
...
function handleSuccess() {
    if(props.data?.id) {
        toast.success(" Cardapio editado com sucesso!");
    } else {
...
const loadData = async () => {
    try {
        const response = await axios.get(`/admin/cardapio/${props.data.id}`);
        ...
        toast.error('Não foi possível recuperar os dados do  Cardapio.');
    } finally {
        ready.value = true;
    }
}
...
onMounted(async () => {
    if (props.data?.id) {
        acao.value = `/admin/cardapio/${props.data.id}`;
        await loadData();
    } else {
        ready.value = true;
    }
