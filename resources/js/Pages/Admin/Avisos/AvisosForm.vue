        ...
        <form @submit.prevent="submit">
            ...
            <div class="mb-4">
                ...
                <VueDatePicker
                    ...
                    class="w-full"
                />
                <InputError :message="errors.data_expiração"/>
            </div>
...
const acao = ref('/admin/avisos/create');
    ...
    axios.post(acao.value, form.value)
        ...
        .catch(error => {
            ...
            if (error.response) {
                ...
                const message = data.message || "Ocorreu um erro ao salvar Avisos.";
                handleError(message);
            } else {
    ...
    try {
        ...
        Object.keys(form.value).forEach(key => {
            if (response.data[key] !== undefined) {
                // Converte o campo ativo para boolean
                if (key === 'ativo') {
                    form.value[key] = response.data[key] === 1 || response.data[key] === '1' || response.data[key] === true;
                }
                // Converte a data ISO para objeto Date para o VueDatePicker
