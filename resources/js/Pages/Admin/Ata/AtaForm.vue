    ...
    <div class="m-2" v-if="ready">
        <form @submit.prevent="submit">
            ...
            <div class="mb-4 p-4 border border-gray-200 rounded-md bg-gray-50" v-if="!readOnly">
                ...
                <div v-if="selectedFileName" class="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-blue-800">
                            <i class="fa fa-file mr-2"></i>
                            <strong>Novo arquivo selecionado:</strong> {{ selectedFileName }}
                        </p>
                        <button
                            type="button"
...
const acao = ref('/admin/atas/create');
...
function submit() {
    ...
    if (selectedFile.value) {
        ...
        // Adicionar título e descrição do arquivo se fornecidos
        if (form.value.titulo_arquivo) {
            formData.append('titulo_arquivo', form.value.titulo_arquivo);
        }
        if (form.value.descricao_arquivo) {
            formData.append('descricao_arquivo', form.value.descricao_arquivo);
        }
    }
    ...
    axios.post(acao.value, formData, {
        ...
        .catch(error => {
            ...
            if (error.response) {
                ...
                const message = data.message || "Ocorreu um erro ao salvar Ata.";
                handleError(message);
            } else {
...
function handleSuccess() {
    if (props.data?.id) {
        toast.success("Ata editada com sucesso!");
    } else {
