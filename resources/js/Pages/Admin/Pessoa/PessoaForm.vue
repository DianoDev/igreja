<template>
    <div class="m-2" v-if="ready">
        <form @submit.prevent="submit">
            ...
            <div class="mb-4">
                <InputLabel for="cpf" value="Cpf" class="required"/>
                <TextInput
                    id="cpf"
                    class="w-full"
                    v-model="form.cpf"
            ...
            <div class="w-full pt-4 mt-4 border-t border-gray-200">
                ...
                <div class="flex justify-center space-x-2" v-if="!readOnly">
                    <button
                        type="submit"
                        class="px-4 py-2 text-white rounded-md bg-blue-500 hover:bg-primary-hover"
                        :disabled="processing"
                    >
...
function submit() {
    ...
    console.log('Dados sendo enviados:', form.value);
    console.log('URL:', acao.value);
    ...
    axios.post(acao.value, form.value)
        ...
        .catch(error => {
            ...
            if (error.response) {
                ...
                const message = data.message || "Ocorreu um erro ao salvar Pessoa.";
                handleError(message);
            } else {
...
function handleSuccess() {
    if(props.data?.id) {
        toast.success(" Pessoa editado com sucesso!");
    } else {
