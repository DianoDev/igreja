    ...
    <div class="relative">
        ...
        <slot>
            <div v-if="checkboxEnabled" class="text-center">
                <h3 class="font-bold text-gray-400">Selected {{selection.selected}} of {{selection.total}} item(s)</h3>
            </div>
        </slot>
        ...
        <div class="flex justify-between gap-4 mt-4 w-100" v-if="total > 0 && !disablePagination">
            <div class="flex items-center gap-3">
                <div class="col-span-2">
                    <select class="block w-full min-w-[200px] px-3 py-1.5 text-sm border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" v-model="per_page" @change="changePerPage">
                ...
                <div class="col-span-3">
                    <p class="text-sm">
                        Exibindo registros de {{from}} a {{ to }}
                    </p>
                </div>
            </div>
        ...
        <div v-if="total === 0" class="py-4 text-center">
            <p>
                Sem registros para exibir
            </p>
        </div>
        <div v-if="error" class="py-2 text-center">
            <p class="px-4 py-2 text-sm text-red-700 bg-red-100 rounded">
                <i class="fa fa-exclamation-circle"></i> Erro ao tentar exibir dados.
                <strong>
                    <button @click="loadData" type="button" class="px-2 py-1 ml-2 text-sm text-white bg-red-600 rounded">
